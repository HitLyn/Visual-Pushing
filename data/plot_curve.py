...
def get_data_frame(df_list, num_list):
    ...
    for num in num_list:
        for i in range(3):
            list_index.append(num * 3 + i)
    # concatnate df to plot
    for index in list_index:
        df = df_list[index]
        # embed()
        dict_data = {'tag': [job_dirs[index].split('/')[7]] * 963, 'step': np.linspace(2450,2409950,num = 963)/1e6,
                     'success': df['eval/success_rate'][:963]}
...
if __name__ == "__main__":
    ...
    job_dirs = glob.glob(f"{push_home}/log_files/test_data/*/*/event*")
    # embed()
    df_list = load_tf_jobs()
    line_name_list = [job.split('/')[7] for job in job_dirs[::5]]
    print(line_name_list)
    # df = get_data_frame(df_list, [1,2,3,5,6,8])
    # plot
    df1 = get_data_frame(df_list, [0,1])
    # df2 = get_data_frame(df_list, [0, 3, 4, 6])
    # df3 = get_data_frame(df_list, [3, 7, 9])

    fig = plt.figure(figsize=(30, 8))
    # 1
    ax1 = plt.subplot2grid((4, 15), (0, 0), colspan=5, rowspan=4)
    # ax1 = plt.subplot(1,3,1)
    sns.lineplot(data=df1, x="step", y="average_success_rate", hue='tag')
    # ax1.set(xlabel = None, ylabel = 'Success rate')
    # ax1.xaxis.set_visible(False)
    ax1.set_xlabel('Steps (M)', fontsize=25)
    ax1.set_ylabel('Success rate', fontsize=25)
    ax1.set_title('Input Sequence Length', fontdict={'fontsize': 30, 'fontweight': "medium"}, pad=5.0)
    ax1.set_xlim(0, 2.5)
    ax1.set_ylim(0, 1)
    # ax1.set_xticks([])
    ...
    ax1.legend(frameon=False, loc='lower right', ncol=2, prop={'size': 18})

    # # 2
    # ax2 = plt.subplot2grid((4, 15), (0, 5), colspan=5, rowspan=4)
    # # ax2 = plt.subplot(1, 3, 2)
    # sns.lineplot(data=df2, x="step", y="average_success_rate", hue='tag')
    # ax2.set_title('Input Modal', fontdict={'fontsize': 30, 'fontweight': "medium"}, pad=1.0)
    # # ax2.set(xlabel = None, ylabel = 'Success rate')
    # ax2.set_ylabel('Success rate', fontsize=25)
    ...
    # # ax2.set_xticks([])
    # ax2.legend(frameon=False, loc='lower right', ncol=2, prop={'size': 18})
    #
    # # 3
    # ax3 = plt.subplot2grid((4, 15), (0, 10), colspan=5, rowspan=4)
    # # ax3 = plt.subplot(1, 3, 3)
    # sns.lineplot(data=df3, x="step", y="average_success_rate", hue='tag')
    # ax3.set_title('PCA Components', fontdict={'fontsize': 30, 'fontweight': "medium"}, pad=1.0)
    # # ax3.set(xlabel = 'Eipsodes', ylabel = 'Success rate', fontsize = 12)
    # ax3.set_xlabel('Episodes', fontsize=25)
    # ax3.set_ylabel('Success rate', fontsize=25)
    # ax3.set_xlim(0, 1000)
    # ax3.set_ylim(0, 1)
    # ax3.grid(color='white')
    # ax3.tick_params(axis='both', which='major', labelsize=25)
    # ax3.legend(frameon=False, loc='lower right', ncol=2, prop={'size': 18})
